...
import { Trash2, Plus, FileText, ListChecks } from 'lucide-react';
...
export default function CriteriosEvaluacionManager() {
  ...
  const handleEliminar = async (id, grupo) => {
    if (!confirm('¿Está seguro de eliminar este criterio de evaluación?')) return;
    
    try {
      setDeleteLoading(id);
      const response = await fetch(`/api/rubric/delete/${id}`, {
        method: 'DELETE'
      });
  ...
  const TablaCriterios = ({ criterios, grupo, titulo }) => (
